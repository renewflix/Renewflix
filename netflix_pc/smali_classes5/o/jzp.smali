.class public final Lo/jzp;
.super Lo/jzh;
.source ""


# instance fields
.field private final c:Lo/jyY;

.field private final d:Lkotlin/text/Regex;


# direct methods
.method public constructor <init>(Lo/jzb;Lo/jyY;Lkotlin/text/Regex;Lo/jyS$a;)V
    .locals 1

    const-string v0, ""

    invoke-static {p1, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-static {p2, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-static {p4, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    .line 22
    invoke-virtual {p2}, Lo/jyY;->b()Lo/jyY$d;

    move-result-object v0

    invoke-direct {p0, p1, v0}, Lo/jzh;-><init>(Lo/jzb;Lo/jyY$d;)V

    .line 19
    iput-object p2, p0, Lo/jzp;->c:Lo/jyY;

    .line 20
    iput-object p3, p0, Lo/jzp;->d:Lkotlin/text/Regex;

    .line 25
    new-instance p1, Lo/iSr;

    invoke-virtual {p4}, Lo/jyS$a;->h()I

    move-result p3

    invoke-virtual {p4}, Lo/jyS$a;->g()I

    move-result p4

    invoke-direct {p1, p3, p4}, Lo/iSr;-><init>(II)V

    sget-object p3, Lo/jyx;->v:Lo/jyt;

    .line 24
    new-instance p4, Lo/jzJ$c;

    invoke-direct {p4, p1, p3}, Lo/jzJ$c;-><init>(Lo/iSr;Lo/jyt;)V

    invoke-static {p4}, Lo/iPs;->e(Ljava/lang/Object;)Ljava/util/List;

    move-result-object p1

    check-cast p1, Ljava/util/Collection;

    invoke-virtual {p2, p1}, Lo/jyY;->d(Ljava/util/Collection;)V

    return-void
.end method


# virtual methods
.method public final a()Z
    .locals 1

    const/4 v0, 0x0

    return v0
.end method

.method public final c()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$ClosingAction;
    .locals 1

    .line 33
    sget-object v0, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$ClosingAction;->d:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$ClosingAction;

    return-object v0
.end method

.method public final d(Lo/jyS$a;Lo/jzb;)Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;
    .locals 4

    const-string v0, ""

    invoke-static {p1, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-static {p2, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    .line 37
    invoke-virtual {p1}, Lo/jyS$a;->j()I

    move-result p2

    const/4 v0, -0x1

    if-eq p2, v0, :cond_0

    .line 38
    sget-object p1, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;->c:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;

    invoke-static {}, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;->d()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;

    move-result-object p1

    return-object p1

    .line 1067
    :cond_0
    iget p2, p1, Lo/jyS$a;->a:I

    if-lez p2, :cond_1

    .line 1068
    iget-object p2, p1, Lo/jyS$a;->c:Lo/jyS;

    invoke-static {p2}, Lo/jyS;->a(Lo/jyS;)Ljava/util/List;

    move-result-object p2

    iget v0, p1, Lo/jyS$a;->a:I

    add-int/lit8 v0, v0, -0x1

    invoke-interface {p2, v0}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object p2

    check-cast p2, Ljava/lang/String;

    goto :goto_0

    :cond_1
    const/4 p2, 0x0

    :goto_0
    if-nez p2, :cond_2

    .line 42
    sget-object p1, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;->c:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;

    invoke-static {}, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;->e()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;

    move-result-object p1

    return-object p1

    .line 43
    :cond_2
    invoke-virtual {p0}, Lo/jzh;->b()Lo/jzb;

    move-result-object v0

    invoke-interface {v0, p1}, Lo/jzb;->e(Lo/jyS$a;)Lo/jzb;

    move-result-object v0

    invoke-virtual {p0}, Lo/jzh;->b()Lo/jzb;

    move-result-object v1

    invoke-static {v0, v1}, Lo/jzf;->d(Lo/jzb;Lo/jzb;)Z

    move-result v0

    if-nez v0, :cond_3

    .line 44
    sget-object p1, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;->c:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;

    invoke-static {}, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;->e()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;

    move-result-object p1

    return-object p1

    .line 47
    :cond_3
    iget-object v0, p0, Lo/jzp;->d:Lkotlin/text/Regex;

    if-nez v0, :cond_4

    sget-object v0, Lo/jzc;->b:Lo/jzc;

    invoke-virtual {p0}, Lo/jzh;->b()Lo/jzb;

    move-result-object v0

    invoke-static {p1, v0}, Lo/jzc;->c(Lo/jyS$a;Lo/jzb;)I

    move-result v0

    const/4 v1, 0x2

    if-lt v0, v1, :cond_4

    .line 48
    sget-object p1, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;->c:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;

    invoke-static {}, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;->e()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;

    move-result-object p1

    return-object p1

    .line 49
    :cond_4
    iget-object v0, p0, Lo/jzp;->d:Lkotlin/text/Regex;

    if-eqz v0, :cond_5

    invoke-static {v0, p2}, Lkotlin/text/Regex;->c(Lkotlin/text/Regex;Ljava/lang/CharSequence;)Lo/iTF;

    move-result-object p2

    if-eqz p2, :cond_5

    .line 50
    sget-object p1, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;->c:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;

    invoke-static {}, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;->e()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;

    move-result-object p1

    return-object p1

    .line 53
    :cond_5
    invoke-virtual {p1}, Lo/jyS$a;->a()Ljava/lang/String;

    move-result-object p2

    invoke-interface {p2}, Ljava/lang/CharSequence;->length()I

    move-result p2

    if-lez p2, :cond_6

    .line 54
    iget-object p2, p0, Lo/jzp;->c:Lo/jyY;

    .line 55
    new-instance v0, Lo/iSr;

    invoke-virtual {p1}, Lo/jyS$a;->h()I

    move-result v1

    add-int/lit8 v1, v1, 0x1

    invoke-virtual {p0}, Lo/jzh;->b()Lo/jzb;

    move-result-object v2

    invoke-virtual {p1}, Lo/jyS$a;->a()Ljava/lang/String;

    move-result-object v3

    invoke-static {v2, v3}, Lo/jzf;->d(Lo/jzb;Ljava/lang/CharSequence;)I

    move-result v2

    add-int/2addr v1, v2

    invoke-virtual {p1}, Lo/jyS$a;->g()I

    move-result p1

    invoke-direct {v0, v1, p1}, Lo/iSr;-><init>(II)V

    .line 56
    sget-object p1, Lo/jyx;->v:Lo/jyt;

    .line 54
    new-instance v1, Lo/jzJ$c;

    invoke-direct {v1, v0, p1}, Lo/jzJ$c;-><init>(Lo/iSr;Lo/jyt;)V

    invoke-static {v1}, Lo/iPs;->e(Ljava/lang/Object;)Ljava/util/List;

    move-result-object p1

    check-cast p1, Ljava/util/Collection;

    invoke-virtual {p2, p1}, Lo/jyY;->d(Ljava/util/Collection;)V

    .line 60
    :cond_6
    sget-object p1, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;->c:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;

    invoke-static {}, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;->d()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;

    move-result-object p1

    return-object p1
.end method

.method public final d(Lo/jyS$a;)Z
    .locals 1

    .line 0
    const-string v0, ""

    invoke-static {p1, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    const/4 p1, 0x1

    return p1
.end method

.method public final e(Lo/jyS$a;)I
    .locals 1

    const-string v0, ""

    invoke-static {p1, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    .line 64
    invoke-virtual {p1}, Lo/jyS$a;->g()I

    move-result p1

    return p1
.end method

.method public final e()Lo/jyt;
    .locals 1

    .line 68
    sget-object v0, Lo/jyA;->n:Lo/jyt;

    return-object v0
.end method
