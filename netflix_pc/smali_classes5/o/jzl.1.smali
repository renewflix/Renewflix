.class public final Lo/jzl;
.super Lo/jzh;
.source ""


# instance fields
.field private a:I

.field private final b:Ljava/lang/String;

.field private final c:Lo/jyY;

.field private final d:Lkotlin/text/Regex;


# direct methods
.method public constructor <init>(Lo/jzb;Lo/jyY;Ljava/lang/String;)V
    .locals 1

    const-string v0, ""

    invoke-static {p1, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-static {p2, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-static {p3, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    .line 18
    invoke-virtual {p2}, Lo/jyY;->b()Lo/jyY$d;

    move-result-object v0

    invoke-direct {p0, p1, v0}, Lo/jzh;-><init>(Lo/jzb;Lo/jyY$d;)V

    .line 17
    iput-object p2, p0, Lo/jzl;->c:Lo/jyY;

    .line 18
    iput-object p3, p0, Lo/jzl;->b:Ljava/lang/String;

    .line 23
    new-instance p1, Ljava/lang/StringBuilder;

    invoke-direct {p1}, Ljava/lang/StringBuilder;-><init>()V

    const-string p2, "^ {0,3}"

    invoke-virtual {p1, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p1, p3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string p2, "+ *$"

    invoke-virtual {p1, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    new-instance p2, Lkotlin/text/Regex;

    invoke-virtual {p1}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object p1

    invoke-direct {p2, p1}, Lkotlin/text/Regex;-><init>(Ljava/lang/String;)V

    iput-object p2, p0, Lo/jzl;->d:Lkotlin/text/Regex;

    const/4 p1, -0x1

    .line 25
    iput p1, p0, Lo/jzl;->a:I

    return-void
.end method


# virtual methods
.method public final a()Z
    .locals 1

    const/4 v0, 0x0

    return v0
.end method

.method public final c()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$ClosingAction;
    .locals 1

    .line 32
    sget-object v0, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$ClosingAction;->d:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$ClosingAction;

    return-object v0
.end method

.method public final d(Lo/jyS$a;Lo/jzb;)Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;
    .locals 3

    const-string v0, ""

    invoke-static {p1, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-static {p2, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    .line 37
    invoke-virtual {p1}, Lo/jyS$a;->h()I

    move-result p2

    iget v1, p0, Lo/jzl;->a:I

    if-ge p2, v1, :cond_0

    .line 38
    sget-object p1, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;->c:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;

    invoke-static {}, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;->d()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;

    move-result-object p1

    return-object p1

    .line 42
    :cond_0
    invoke-virtual {p1}, Lo/jyS$a;->j()I

    move-result p2

    const/4 v1, -0x1

    if-eq p2, v1, :cond_1

    .line 43
    sget-object p1, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;->c:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;

    invoke-static {}, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;->d()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;

    move-result-object p1

    return-object p1

    .line 46
    :cond_1
    sget-object p2, Lo/jyQ;->e:Lo/jyQ;

    invoke-virtual {p1}, Lo/jyS$a;->j()I

    move-result p2

    if-ne p2, v1, :cond_5

    .line 48
    invoke-virtual {p0}, Lo/jzh;->b()Lo/jzb;

    move-result-object p2

    invoke-static {p2, p1}, Lo/jzf;->d(Lo/jzb;Lo/jyS$a;)Lo/jzb;

    move-result-object p2

    .line 49
    invoke-virtual {p0}, Lo/jzh;->b()Lo/jzb;

    move-result-object v0

    invoke-static {p2, v0}, Lo/jzf;->d(Lo/jzb;Lo/jzb;)Z

    move-result v0

    if-nez v0, :cond_2

    .line 50
    sget-object p1, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;->c:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;

    invoke-static {}, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;->e()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;

    move-result-object p1

    return-object p1

    .line 53
    :cond_2
    invoke-virtual {p1}, Lo/jyS$a;->g()I

    move-result v0

    .line 54
    iput v0, p0, Lo/jzl;->a:I

    .line 56
    invoke-virtual {p1}, Lo/jyS$a;->a()Ljava/lang/String;

    move-result-object v1

    invoke-static {p2, v1}, Lo/jzf;->c(Lo/jzb;Ljava/lang/CharSequence;)Ljava/lang/CharSequence;

    move-result-object p2

    .line 1073
    iget-object v1, p0, Lo/jzl;->d:Lkotlin/text/Regex;

    invoke-virtual {v1, p2}, Lkotlin/text/Regex;->e(Ljava/lang/CharSequence;)Z

    move-result p2

    if-eqz p2, :cond_3

    .line 58
    iget-object p2, p0, Lo/jzl;->c:Lo/jyY;

    new-instance v1, Lo/iSr;

    invoke-virtual {p1}, Lo/jyS$a;->h()I

    move-result v2

    add-int/lit8 v2, v2, 0x1

    invoke-virtual {p1}, Lo/jyS$a;->g()I

    move-result p1

    invoke-direct {v1, v2, p1}, Lo/iSr;-><init>(II)V

    .line 59
    sget-object p1, Lo/jyx;->f:Lo/jyt;

    .line 58
    new-instance v2, Lo/jzJ$c;

    invoke-direct {v2, v1, p1}, Lo/jzJ$c;-><init>(Lo/iSr;Lo/jyt;)V

    invoke-static {v2}, Lo/iPs;->e(Ljava/lang/Object;)Ljava/util/List;

    move-result-object p1

    check-cast p1, Ljava/util/Collection;

    invoke-virtual {p2, p1}, Lo/jyY;->d(Ljava/util/Collection;)V

    .line 60
    sget-object p1, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;->c:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;

    invoke-static {}, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;->e()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;

    move-result-object p1

    invoke-virtual {p0, v0, p1}, Lo/jzh;->e(ILorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;)V

    goto :goto_0

    .line 62
    :cond_3
    new-instance p2, Lo/iSr;

    invoke-virtual {p1}, Lo/jyS$a;->h()I

    move-result v1

    add-int/lit8 v1, v1, 0x1

    invoke-virtual {p0}, Lo/jzh;->b()Lo/jzb;

    move-result-object v2

    invoke-virtual {p1}, Lo/jyS$a;->a()Ljava/lang/String;

    move-result-object p1

    invoke-static {v2, p1}, Lo/jzf;->d(Lo/jzb;Ljava/lang/CharSequence;)I

    move-result p1

    add-int/2addr v1, p1

    invoke-static {v1, v0}, Ljava/lang/Math;->min(II)I

    move-result p1

    invoke-direct {p2, p1, v0}, Lo/iSr;-><init>(II)V

    .line 63
    invoke-virtual {p2}, Lo/iSv;->d()I

    move-result p1

    invoke-virtual {p2}, Lo/iSv;->b()I

    move-result v0

    if-ge p1, v0, :cond_4

    .line 64
    iget-object p1, p0, Lo/jzl;->c:Lo/jyY;

    .line 65
    sget-object v0, Lo/jyx;->h:Lo/jyt;

    .line 64
    new-instance v1, Lo/jzJ$c;

    invoke-direct {v1, p2, v0}, Lo/jzJ$c;-><init>(Lo/iSr;Lo/jyt;)V

    invoke-static {v1}, Lo/iPs;->e(Ljava/lang/Object;)Ljava/util/List;

    move-result-object p2

    check-cast p2, Ljava/util/Collection;

    invoke-virtual {p1, p2}, Lo/jyY;->d(Ljava/util/Collection;)V

    .line 69
    :cond_4
    :goto_0
    sget-object p1, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;->c:Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;

    invoke-static {}, Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e$a;->d()Lorg/intellij/markdown/parser/markerblocks/MarkerBlock$e;

    move-result-object p1

    return-object p1

    .line 83
    :cond_5
    new-instance p1, Lorg/intellij/markdown/MarkdownParsingException;

    invoke-direct {p1, v0}, Lorg/intellij/markdown/MarkdownParsingException;-><init>(Ljava/lang/String;)V

    throw p1
.end method

.method public final d(Lo/jyS$a;)Z
    .locals 1

    .line 0
    const-string v0, ""

    invoke-static {p1, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    const/4 p1, 0x1

    return p1
.end method

.method public final e(Lo/jyS$a;)I
    .locals 1

    const-string v0, ""

    invoke-static {p1, v0}, Lo/iRL;->b(Ljava/lang/Object;Ljava/lang/String;)V

    .line 28
    invoke-virtual {p1}, Lo/jyS$a;->g()I

    move-result p1

    return p1
.end method

.method public final e()Lo/jyt;
    .locals 1

    .line 77
    sget-object v0, Lo/jyA;->f:Lo/jyt;

    return-object v0
.end method
